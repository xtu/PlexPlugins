#subhd.net
#Subtitles service allowed by www.subhd.net
...
def Start():
  HTTP.CacheTime = 0
  HTTP.Headers['User-agent'] = SUBHD_PLEX_USERAGENT
  Log.Debug("Subhd Agent Start")
...
def fetchSubtitle(url):
  ...
  headers = {'Content-Type' : 'application/x-www-form-urlencoded', 'User-Agent' : SUBHD_PLEX_USERAGENT}
  params = urllib.urlencode(post_data)

  req = urllib2.Request(SUBHD_API, params, headers)
  response = urllib2.urlopen(req)
  json_data = json.load(response)

  if len(json_data) == 0:
    Log('Wrong response from SUBHD API')
    return
...
class SubhdAgentMovies(Agent.Movies):
  name = 'Subhd.com'
  ...
  def search(self, results, media, lang):
    Log.Debug("SubhdAgentMovies.search")
  ...
  def update(self, metadata, media, lang):
    Log.Debug("SubhdAgentMovies.update")    
...
class SubhdAgentTVShows(Agent.TV_Shows):
  name = 'Subhd.com'
  languages = [Locale.Language.Chinese]
  primary_provider = False
  ...
  def search(self, results, media, lang, manual):
    Log.Debug("SubhdAgentTVShows.search")
    results.Append(MetadataSearchResult(
      id    = "null",    
      score = 100
    ))
  ...
  def update(self, metadata, media, lang, force):
    Log.Debug("SubhdAgentTVShows.update")
