#subom.net
#Subtitles service allowed by www.subom.net
...
SUBOM_PLEX_USERAGENT = 'plexapp.com v9.0'
subtitleExt       = ['utf','utf8','utf-8','sub','srt','smi','rt','ssa','aqt','jss','ass','idx']
 
def Start():
  HTTP.CacheTime = 0
  HTTP.Headers['User-agent'] = SUBOM_PLEX_USERAGENT
  Log.Debug("Subom Agent Start")
...
def fetchSubtitle(url):
  ...
  headers = {'Content-Type' : 'application/x-www-form-urlencoded', 'User-Agent' : SUBOM_PLEX_USERAGENT}
  params = urllib.urlencode(post_data)

  req = urllib2.Request(SUBOM_API, params, headers)
  response = urllib2.urlopen(req)
  json_data = json.load(response)

  if len(json_data) == 0:
    Log('Wrong response from SUBOM API')
    return
...
class SubomAgentMovies(Agent.Movies):
  name = 'Subom.net'
  ...
  def search(self, results, media, lang):
    Log.Debug("SubomAgentMovies.search")
  ...
  def update(self, metadata, media, lang):
    Log.Debug("SubomAgentMovies.update")    
...
class SubomAgentTVShows(Agent.TV_Shows):
  name = 'Subom.net'
  languages = [Locale.Language.Chinese]
  primary_provider = False
  ...
  def search(self, results, media, lang, manual):
    Log.Debug("SubomAgentTVShows.search")
    results.Append(MetadataSearchResult(
      id    = "null",    
      score = 100
    ))
  ...
  def update(self, metadata, media, lang, force):
    Log.Debug("SubomAgentTVShows.update")
